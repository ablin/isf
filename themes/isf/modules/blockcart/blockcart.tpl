									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<a class="cart-images" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
											{if preg_match("/-default/", $product.id_image)}
												<div class="filigrane">
													<span>
														{l s='Non contractual photo' mod='blockcart'}
													</span>
												</div>
											{/if}
											<img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default', $product.id_product)}" alt="{$product.name|escape:'html':'UTF-8'}" />
										</a>
										<div class="cart-info">
		...
		<div class="clearfix">
			<div class="layer_cart_product col-xs-12 col-md-6">
				...
				<div class="layer_cart_product_info">
					...
					<div>
						<span id="layer_cart_product_alert"></span>
					</div>
				</div>
			</div>
