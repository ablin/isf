        ...
        if ($category_type === 'products') {
            ...
            if ($interval) {
                ...
                foreach ($categories as $category) {
                    $full_path .=
                    (($n < $n_categories || $link_on_the_item) ? '<a href="'.Tools::safeOutput($context->link->getCategoryLink((int)$category['id_category'], $category['link_rewrite'])).'" title="'.htmlentities(strip_tags($category['description']), ENT_NOQUOTES, 'UTF-8').'" data-gg="">' : '').
                    htmlentities(strip_tags($category['description']), ENT_NOQUOTES, 'UTF-8').
