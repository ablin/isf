...
class AuthController extends AuthControllerCore
{
    ...
    protected function processSubmitLogin()
    {
        ...
        if (empty($login)) {
            $this->errors[] = Tools::displayError('Login is required.');
        } elseif (empty($passwd)) {
            ...
            try {
                ...
                if ($datas && $datas->contact_trouve == 1 && $datas->identification == 1) {
                    ...
                    $this->context->cookie->__set('orderNumberRequired', $datas->orderNumberRequired);
            ...
            if (Configuration::get('PS_CART_FOLLOWING') && (empty($this->context->cookie->id_cart) || Cart::getNbProducts($this->context->cookie->id_cart) == 0) && $id_cart = (int)Cart::lastNoneOrderedCart($this->context->customer->id)) {
                if (count($datas->panier->references) > 0) {
                    ...
                    foreach ($datas->panier->references as $reference) {
